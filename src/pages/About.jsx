...
export function About() {
  ...
  const videoRef = useRef(null); // Reference to the video element
    const [isPlaying, setIsPlaying] = useState(false); // Play/Pause state (default to false)
    const [isMuted, setIsMuted] = useState(true); // Mute/Unmute state (default to muted)
    const [currentTime, setCurrentTime] = useState(0); // To track video current time
    const [duration, setDuration] = useState(0); // To track video duration
    const [isHovered, setIsHovered] = useState(false); // To control hover state for range bar visibility
