...
import "@/styles/animations.css"; // Ensure you have a CSS file for animations

function EmailTool() {
  return (
    <div className="min-h-[350vh] bg-gray-50 text-gray-900">
      {/* Header Section with fade-in from left */}
      ...
      {/* Hero Section with AWS-like Gradient Animation (Fade-in from top) */}
      <section className="relative w-full h-[16cm] flex items-center justify-start p-10 overflow-hidden bg-white animate-fade-in-up">
        ...
        <div className="relative z-10 text-left max-w-5xl">
          ...
          <p className="max-w-8xl text-5x1 mt-4 text-gray-700 animate-fade-in-left">
      ...
      {/* Email Providers Section (Fade-in from left) */}
      <section className="mx-[2cm] min-h-[150vh] mt-10 grid grid-cols-1 md:grid-cols-3 gap-6 p-6 animate-fade-in-left">
        {[
          ...
          <div key={index} className="h-[9cm] bg-white p-6 rounded-lg shadow-md text-center hover:shadow-lg transition duration-300 h-[14cm] group">
      ...
      {/* Comparison Table (Fade-in from right) */}
      ...
      {/* Call to Action (Fade-in from bottom) */}
